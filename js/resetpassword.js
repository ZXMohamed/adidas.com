...
import { get, ref, getDatabase } from "https://www.gstatic.com/firebasejs/11.0.2/firebase-database.js"
...
function handleVerifyEmail(auth, actionCode, continueUrl, lang) {
    ...
    applyActionCode(auth, actionCode).then((resp) => {
        ...
        next.addEventListener("click", () => {
            ...
            //^
            get(ref(db, "/sitetype")).then((type) => {
                if (type.exists()) {console.log(type.val());
                    if (type.val() == "js") {
                        Routing.goto(Routing.pages.main, true);
                    } else if (type.val() == "react") {
                        location.replace("https://adidas-shop-95690.web.app/")
